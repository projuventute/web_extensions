if (typeof window.rnw === 'object' && typeof window.rnw.tamaro === 'object') {
   ...
   //       case 'ezs':       // Einzahlungsschein
      ...
      if (typeof window.TMSProcessing === 'object') {
         ...
         window.rnw.tamaro.events.paymentComplete.subscribe(function(event) {
            try {
               ...
               window.console.log('[raiseNow config] error:');
      ...
      window.TMSHelper.console('[raiseNow config] -> warning: waited too long, RaiseNow config not triggered');
   } else {
      window.TMSHelper.console('[raiseNow config] -> info: RaiseNow widget core not ready, trying again in 0.5 seconds...');
